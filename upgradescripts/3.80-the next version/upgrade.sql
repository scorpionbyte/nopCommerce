...
--new stored procedure gor getting category tree
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[Get_CategoryTree]') AND OBJECTPROPERTY(object_id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[Get_CategoryTree]
GO

CREATE PROCEDURE [dbo].[Get_CategoryTree]
(
    @ShowHidden         BIT = 0,
...
BEGIN
    ...
    --get category tree
    ;WITH [CategoryTree] 
    AS (SELECT dbo.[nop_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[nop_padright] ([Category].[Id], '0', @lengthId) AS [Order], *
        FROM [Category] WHERE [Category].[ParentCategoryId] = 0
        UNION ALL
        SELECT [CategoryTree].[Order] + '|' + dbo.[nop_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[nop_padright] ([Category].[Id], '0', @lengthId) AS [Order], [Category].*
        FROM [Category]
        INNER JOIN [CategoryTree] ON [CategoryTree].[Id] = [Category].[ParentCategoryId])
    INSERT INTO #OrderedCategoryIds ([CategoryId]) 
    SELECT [CategoryTree].[Id] 
    FROM [CategoryTree]
        
    --filter results
    WHERE [CategoryTree].[Deleted] = 0 
    AND (@ShowHidden = 1 OR [CategoryTree].[Published] = 1) 
    AND (@Name IS NULL OR @Name = '' OR [CategoryTree].[Name] LIKE ('%' + @Name + '%'))
    AND (@ShowHidden = 1 OR @FilteredCustomerRoleIdsCount  = 0 OR [CategoryTree].[SubjectToAcl] = 0 
        OR EXISTS (SELECT 1 FROM #FilteredCustomerRoleIds [roles] WHERE [roles].[CustomerRoleId] IN 
            (SELECT [acl].[CustomerRoleId] FROM [AclRecord] acl WITH (NOLOCK) WHERE [acl].[EntityId] = [CategoryTree].[Id] AND [acl].[EntityName] = 'Category')
        )
    )
    AND (@StoreId = 0 OR [CategoryTree].[LimitedToStores] = 0 
        OR EXISTS (SELECT 1 FROM [StoreMapping] sm WITH (NOLOCK)
			WHERE [sm].[EntityId] = [CategoryTree].[Id] AND [sm].[EntityName] = 'Category' AND [sm].[StoreId] = @StoreId
		)
    )
    ORDER BY [CategoryTree].[Order]
